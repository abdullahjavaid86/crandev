...
| D10 | **Reading panel (prototype).** Fills the otherwise-dead right gutter past 1400px with a live "currently reading" block. Keep, change, or drop. If kept, lift the IntersectionObserver into one provider — the rail and the panel currently run one each over the same seven sections.                                                                                                                                                   | 2026-08-09 | open     |
| D9  | **Ambient background + hero shape.** Both live behind a dev-only picker (bottom-right). Backgrounds: `fields` (blurred gradients), `lattice` (projected point field), `grid` (perspective plane), `none`. Hero shapes: `knot`, `globe`, `icosahedron`, `helix`, `vortex`. Pick one of each, then delete the losers plus `useDomFlag`, `useBackground`, `useHeroShape`, `DevVariantPicker`, `BackgroundLayer` and the ThemeScript lines. | 2026-08-08 | open     |
