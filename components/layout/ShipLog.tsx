 ...
 * composed by the header, carries the same information as a 2px bar.
 *
 * Decorative: `aria-hidden`. Every label and number here is already announced
 * by the section's own heading and eyebrow, so exposing the rail would make a
 * screen reader read the page's table of contents twice on every route.
 */
...
function Rail({ sections, className }: ShipLogProps) {
   ...
   *
   * Unsmoothed on purpose: a spring here would be a second set of timing
   * constants for a 1px line whose jitter is invisible, and under reduced
   * motion the raw value is what we would fall back to anyway.
   */
  ...
  return (
    <div
      ...
      className={cn(
        "pointer-events-none fixed top-1/2 left-4 z-30 -translate-y-1/2",
        className,
      )}
    >
      <ol className="relative flex flex-col gap-4">
        {/* Track and fill run from the first node's centre to the last's.
            ...
            exact centres, so the geometry holds at any commit count. */}
        <span className="absolute top-2 bottom-2 left-[3px] w-px -translate-x-1/2 bg-line" />
        <motion.span
          style={{ scaleY: scrollYProgress }}
          className="absolute top-2 bottom-2 left-[3px] w-px origin-top -translate-x-1/2 bg-line-strong"
        />
        ...
        {commits.map((commit) => {
          ...
          return (
            <li
              key={commit.id}
              className={cn(
                "relative flex h-4 items-center gap-2",
                settle,
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  settle,
                  isCurrent ? "scale-[1.35] bg-accent" : "scale-100 bg-line-strong",
                )}
              />
              ...
              {/* Mono metadata appears only where the gutter can hold it.
                  ...
                  signal anyway. */}
              <span className="hidden font-mono text-xs tracking-[0.18em] uppercase min-[1400px]:flex min-[1400px]:items-baseline min-[1400px]:gap-2">
